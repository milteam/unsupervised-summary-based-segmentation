...
if $RUN_NAIVE; then
    ...
    # done
    # nice bash ./run_naive.sh ${DIALSEG711_PATH} dialseg
...
if $RUN_TILING; then
    ...
    # CUDA_VISIBLE_DEVICES=7 nice bash ./run_tiling.sh $DIALSEG711_PATH dialseg
...
if $RUN_SUM; then
    ...
    nice bash run_sumseg.sh $QMSUM_VAL_PATH $QMSUM_TEST_PATH qmsum > ss-qmsum.txt
